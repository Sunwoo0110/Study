...
### 여기서 잠깐!! 궁금증 해결하고 가기 ᕦ( •ᗜ•)ᕤ
### 왜 현대 OS 대부분이 선점형일까?
- CPU-bound 프로세스 1개 (계속 CPU 필요)
- I/O-bound 프로세스 10개 (짧게 CPU 쓰고 곧바로 I/O로 block)
- 만약, CPU 프로세스가 먼저 실행되고 있으면 I/O 프로세스가 계속 ready queue에 대기 -> 응답 시간 증가
  - CPU 프로세스: I/O 거의 없음, 오래 계산
  - I/O 프로세스: CPU 짧게 사용, 금방 block
- 따라서, 현대 OS는 I/O 프로세스에 높은 우선순위를 부여하여 사용자 체감 성능을 향상 시키고, 실제 시스템 반응성도 향상 시킴
  - CPU 실행 타임 슬라이스가 끝나면, I/O에게 넘어가는데 얘는 CPU 짧게 쓰고 바로 block 하기 때문에 전체 성능이 좋아짐
    - block 상태는 cpu를 점유하지 않음! (waiting)
  - MLFQ!!
  
### MLFQ(Multi-Level Feedback Queue)란?
- 앞선 문제를 해결하기 위해서는, 어떤 프로세스가 cpu인지 I/O인지 판단해서 우선순위를 부여해야하는데, 실행 전에는 알 수가 없음
- 그러면! 실행 패턴을 보고 동적으로 판단하자!
- 여러 우선순위 큐를 두고 프로세스를 동적으로 이동시키는 스케줄링
- 새 프로세스는 가장 높은 큐(우선순위가 높고, 타임 슬라이스가 짧)에서 시작
- 타임 슬라이스 내에 못 끝내면, 하위 큐로 내려감
- 즉, 짧고 인터랙티브한 작업(주로 I/O)은 높은 우선순위 큐에서 빠르게 처리되고, CPU를 오래 쓰는 작업은 점점 낮은 큐로 내려감
- 타임 슬라이스 설계 원칙
  - 상위 큐(우선순위 높음): 짧은 타임 슬라이스 -> 인터랙티브/짧은 작업을 빨리 처리해서 응답성 향상
  - 하위 큐(우선순위 낮음): 긴 타임 슬라이스 -> CPU를 오래 쓰는 작업을 몰아서 실행해 컨텍스트 스위치 오버헤드 줄임
- 응답성과 처리량 높아짐
- 단점: Starvation 문제(상위 큐가 비어야 하위 큐가 실행)
  - 따라서, Priority Boost(Aging) 사용: 일정 시간마다 모든 프로세스를 상위 큐로 올려서 실행시킴
