...
- 교착(Deadlock) 이랑 병목(BottleNeck) 차이??
- **교착(Deadlock)**
  - 여러 스레드(프로세스)가 서로 자원을 점유한 채, 서로가 필요한 자원을 기다리며 영원히 멈추는 상태
  -  시스템이 “정지”된 상태!
  -  보통 2개 이상의 스레드/프로세스가 서로 양보 없이 영원히 기다림

- **병목(Bottleneck)**
  -  전체 시스템의 **처리 속도를 느리게 만드는 가장 느린 구간**
  -  속도/효율이 떨어짐 -> 시스템이 “느려짐”(정지가 아님)
  -  일부 구간(코드, 네트워크, I/O 등) 때문에 전체 처리 속도가 제한됨
  -  ex. 웹서버 전체는 빨라졌지만, 데이터베이스 쿼리 하나가 너무 느려서 전체 응답이 느려짐
  -  ex. 병렬처리 시스템에서 순차(직렬)로 처리해야 하는 부분(암달의 법칙의 순차 부분)이 전체 속도를 제한

- 정리하자면~

| 구분    | 교착(Deadlock)       | 병목(Bottleneck)            |
| ----- | ------------------ | ------------------------- |
| 발생 원인 | 자원 점유/대기(상호 의존)    | 특정 부분의 처리 속도/용량이 낮아서      |
| 결과    | **완전 멈춤(진전 없음)**   | **느려짐(처리율 저하, 지연 증가)**    |
| 영향 범위 | 보통 일부/전체 시스템 정지  | 전체 처리 속도 제한(최대치가 병목에 좌우)  |
| 예시    | 교착 상태에 빠진 프로세스/스레드 | 느린 DB, 순차 처리 코드 |
